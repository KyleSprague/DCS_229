...
'''
def test_constructor_with_integer() -> None:   


    with pytest.raises(TypeError) as exception_info:
        String(0)._chars            # this should raise a TypeError
    result   = type(exception_info.value)
    expected = TypeError
    print_test(f'String(0) = expected, \
               result = result, expected = expected')
    assert(result == expected)
'''  
...
# Testing set item for an invalid index 
...
def test_add_on_concatenated_random_str_and_different_random_String(random_string: pytest.fixture, different_random_string: pytest.fixture) -> None:      
    '''    
        Pytest for adding string to different random String 

        (1). stores random_string + different 
        (2) calls print_test with string version of test, result of the actual
        test, and expected result
        (3) assert required by pytest  
    '''  

    addend_one = String(random_string)
    addend_two = String(different_random_string)  
    result = addend_one + addend_two
...
def test_add_class_random_String_and_different_random_str(random_string: pytest.fixture, different_random_string: pytest.fixture): 

    '''    
        Pytest for adding String to different random Str

        (1). stores Random string added to the different random str
        (2) calls print_test with string version of test, result of the actual
        test, and expected result
        (3) assert required by pytest  
    '''  
    ...
    arg_one = String(random_string)
    arg_two = String(different_random_string).__str__()
    result = arg_one.__add__(arg_two)
    expected = random_string + different_random_string
    print_test(f'(arg_one({random_string})).__add__({different_random_string})) for String and str',
        result = result,
        expected = expected)
    assert (result == expected) 

def test_add_class_random_String_and_integer(random_string: pytest.fixture): 

    '''    
        Pytest for adding String to integer of minimal length
    ...
    string = String(random_string)  # first need a construction...
    with pytest.raises(TypeError) as exception_info:
        arg_one.__add__(arg_two)           # this should raise an IndexError
    result   = type(exception_info.value)
    expected = TypeError
    print_test(f'String({random_string}).__add__(0)', \
               result = result, expected = expected)
    assert(result == expected)
